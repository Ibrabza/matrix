...
function getPlainObjectKeys(object) {
	...
	if (!hasGetOwnPropertySymbols) return keys$1;
	var symbols = Object.getOwnPropertySymbols(object);
	if (!symbols.length) return keys$1;
	return [].concat(keys$1, symbols.filter(function(s) {
...
var getOwnPropertyDescriptors = Object.getOwnPropertyDescriptors || function getOwnPropertyDescriptors$1(target) {
	var res = {};
...
var Atom = /* @__PURE__ */ function() {
	...
	*/
	...
	return _createClass(Atom$1, [
		{
			key: "isBeingObserved",
			get: function get$1() {
				return getFlag(this.flags_, Atom$1.isBeingObservedMask_);
			},
			set: function set$1(newValue) {
				this.flags_ = setFlag(this.flags_, Atom$1.isBeingObservedMask_, newValue);
			}
		},
		{
			key: "isPendingUnobservation",
			get: function get$1() {
				return getFlag(this.flags_, Atom$1.isPendingUnobservationMask_);
			},
			set: function set$1(newValue) {
				this.flags_ = setFlag(this.flags_, Atom$1.isPendingUnobservationMask_, newValue);
			}
		},
		{
			key: "diffValue",
			get: function get$1() {
				return getFlag(this.flags_, Atom$1.diffValueMask_) ? 1 : 0;
			},
			set: function set$1(newValue) {
				this.flags_ = setFlag(this.flags_, Atom$1.diffValueMask_, newValue === 1 ? true : false);
			}
		}
	]);
}();
...
function decorate_20223_$1(mthd, context) {
	...
	var ann = this;
	var _createAction = function _createAction$1(m) {
	...
	if (kind == "field") return function(initMthd) {
		var _ann$options_3;
		var mthd$1 = initMthd;
		if (!isAction(mthd$1)) mthd$1 = _createAction(mthd$1);
		if ((_ann$options_3 = ann.options_) != null && _ann$options_3.bound) {
			mthd$1 = mthd$1.bind(this);
			mthd$1.isMobxAction = true;
		}
		return mthd$1;
	};
	if (kind == "method") {
		...
		if ((_this$options_2 = this.options_) != null && _this$options_2.bound) addInitializer(function() {
			var self$1 = this;
			var bound = self$1[name].bind(self$1);
...
function decorate_20223_$3(get$1, context) {
...
function decorate_20223_$4(desc, context) {
	...
	function initializeObservable(target, value) {
		...
		var observable$1 = new ObservableValue(value, (_ann$options_$enhance = (_ann$options_ = ann.options_) == null ? void 0 : _ann$options_.enhancer) != null ? _ann$options_$enhance : deepEnhancer, adm.name_ + "." + name.toString(), false);
		adm.values_.set(name, observable$1);
...
function make_$5(adm, key, descriptor, source) {
	...
	if (descriptor.set) {
		var set$1 = isAction(descriptor.set) ? descriptor.set : createAction(key.toString(), descriptor.set);
	...
	var observableAnnotation$1 = ((_this$options_3 = this.options_) == null ? void 0 : _this$options_3.deep) === false ? observable.ref : observable;
	if (typeof descriptor.value === "function" && (_this$options_4 = this.options_) != null && _this$options_4.autoBind) {
		var _adm$proxy_;
		descriptor.value = descriptor.value.bind((_adm$proxy_ = adm.proxy_) != null ? _adm$proxy_ : adm.target_);
	}
	return observableAnnotation$1.make_(adm, key, descriptor, source);
}
...
var observable = /* @__PURE__ */ assign(createObservable, {
	...
	set: function set$1(initialValues, options) {
		var o = asCreateObservableOptions(options);
...
*/
var computed = function computed$1(arg1, arg2) {
...
function createAction(actionName, fn, autoAction$1, ref) {
	if (autoAction$1 === void 0) autoAction$1 = false;
	if (!isFunction(fn)) die("`action` can only be invoked on functions");
	if (typeof actionName !== "string" || !actionName) die("actions should have valid names, got: '" + actionName + "'");
	function res() {
		return executeAction(actionName, autoAction$1, fn, ref || this, arguments);
	}
...
function allowStateChanges(allowStateChanges$1, func) {
	var prev = allowStateChangesStart(allowStateChanges$1);
	try {
		return func();
	} finally {
...
function allowStateChangesStart(allowStateChanges$1) {
	var prev = globalState.allowStateChanges;
	globalState.allowStateChanges = allowStateChanges$1;
	return prev;
}
...
var ObservableValue = /* @__PURE__ */ function(_Atom) {
	function ObservableValue$1(value, enhancer, name_, notifySpy, equals) {
		var _this;
	...
	_inheritsLoose(ObservableValue$1, _Atom);
	var _proto = ObservableValue$1.prototype;
	_proto.dehanceValue = function dehanceValue(value) {
		if (this.dehancer !== void 0) return this.dehancer(value);
		return value;
	};
	_proto.set = function set$1(newValue) {
...
var ComputedValue = /* @__PURE__ */ function() {
	...
	*/
	...
	var _proto = ComputedValue$1.prototype;
	...
	*/
	...
	return _createClass(ComputedValue$1, [
		{
			key: "isComputing",
			get: function get$1() {
				return getFlag(this.flags_, ComputedValue$1.isComputingMask_);
			},
			set: function set$1(newValue) {
				this.flags_ = setFlag(this.flags_, ComputedValue$1.isComputingMask_, newValue);
			}
		},
		{
			key: "isRunningSetter",
			get: function get$1() {
				return getFlag(this.flags_, ComputedValue$1.isRunningSetterMask_);
			},
			set: function set$1(newValue) {
				this.flags_ = setFlag(this.flags_, ComputedValue$1.isRunningSetterMask_, newValue);
			}
		},
		{
			key: "isBeingObserved",
			get: function get$1() {
				return getFlag(this.flags_, ComputedValue$1.isBeingObservedMask_);
			},
			set: function set$1(newValue) {
				this.flags_ = setFlag(this.flags_, ComputedValue$1.isBeingObservedMask_, newValue);
			}
		},
		{
			key: "isPendingUnobservation",
			get: function get$1() {
				return getFlag(this.flags_, ComputedValue$1.isPendingUnobservationMask_);
			},
			set: function set$1(newValue) {
				this.flags_ = setFlag(this.flags_, ComputedValue$1.isPendingUnobservationMask_, newValue);
			}
		},
		{
			key: "diffValue",
			get: function get$1() {
				return getFlag(this.flags_, ComputedValue$1.diffValueMask_) ? 1 : 0;
			},
			set: function set$1(newValue) {
				this.flags_ = setFlag(this.flags_, ComputedValue$1.diffValueMask_, newValue === 1 ? true : false);
			}
		}
	]);
}();
...
(function(IDerivationState_$1) {
	IDerivationState_$1[IDerivationState_$1["NOT_TRACKING_"] = -1] = "NOT_TRACKING_";
	IDerivationState_$1[IDerivationState_$1["UP_TO_DATE_"] = 0] = "UP_TO_DATE_";
	IDerivationState_$1[IDerivationState_$1["POSSIBLY_STALE_"] = 1] = "POSSIBLY_STALE_";
	IDerivationState_$1[IDerivationState_$1["STALE_"] = 2] = "STALE_";
...
var CaughtException = function CaughtException$1(cause) {
	this.cause = void 0;
	this.cause = cause;
};
...
function checkIfStateModificationsAreAllowed(atom) {
	var hasObservers$1 = atom.observers_.size > 0;
...
function checkIfStateReadsAreAllowed(observable$1) {
	if (!globalState.allowStateReads && globalState.observableRequiresReaction) console.warn("[mobx] Observable '" + observable$1.name_ + "' being read outside a reactive context.");
}
/**
...
var MobXGlobals = function MobXGlobals$1() {
	/**
	* MobXGlobals version.
...
var globalState = /* @__PURE__ */ function() {
	...
	if (global$1.__mobxInstanceCount > 0 && !global$1.__mobxGlobals) canMergeGlobalState = false;
	if (global$1.__mobxGlobals && global$1.__mobxGlobals.version !== new MobXGlobals().version) canMergeGlobalState = false;
	if (!canMergeGlobalState) {
		setTimeout(function() {
			if (!isolateCalled) die(35);
		}, 1);
		return new MobXGlobals();
	} else if (global$1.__mobxGlobals) {
		global$1.__mobxInstanceCount += 1;
		if (!global$1.__mobxGlobals.UNCHANGED) global$1.__mobxGlobals.UNCHANGED = {};
		return global$1.__mobxGlobals;
	} else {
		global$1.__mobxInstanceCount = 1;
		return global$1.__mobxGlobals = /* @__PURE__ */ new MobXGlobals();
	}
}();
function isolateGlobalState() {
	...
	if (canMergeGlobalState) {
		...
		if (--global$1.__mobxInstanceCount === 0) global$1.__mobxGlobals = void 0;
...
function hasObservers(observable$1) {
	return observable$1.observers_ && observable$1.observers_.size > 0;
}
function getObservers(observable$1) {
	return observable$1.observers_;
}
function addObserver(observable$1, node) {
	observable$1.observers_.add(node);
	if (observable$1.lowestObserverState_ > node.dependenciesState_) observable$1.lowestObserverState_ = node.dependenciesState_;
}
function removeObserver(observable$1, node) {
	observable$1.observers_["delete"](node);
	if (observable$1.observers_.size === 0) queueForUnobservation(observable$1);
}
function queueForUnobservation(observable$1) {
	if (observable$1.isPendingUnobservation === false) {
		observable$1.isPendingUnobservation = true;
		globalState.pendingUnobservations.push(observable$1);
	}
}
/**
...
function endBatch() {
	if (--globalState.inBatch === 0) {
		...
		for (var i = 0; i < list.length; i++) {
			...
			if (observable$1.observers_.size === 0) {
				if (observable$1.isBeingObserved) {
					observable$1.isBeingObserved = false;
					observable$1.onBUO();
				}
				if (observable$1 instanceof ComputedValue) observable$1.suspend_();
			}
		}
		globalState.pendingUnobservations = [];
	}
}
function reportObserved(observable$1) {
	checkIfStateReadsAreAllowed(observable$1);
	...
	if (derivation !== null) {
		...
		*/
		if (derivation.runId_ !== observable$1.lastAccessedBy_) {
			observable$1.lastAccessedBy_ = derivation.runId_;
			derivation.newObserving_[derivation.unboundDepsCount_++] = observable$1;
			if (!observable$1.isBeingObserved && globalState.trackingContext) {
				observable$1.isBeingObserved = true;
				observable$1.onBO();
			}
		}
		return observable$1.isBeingObserved;
	} else if (observable$1.observers_.size === 0 && globalState.inBatch > 0) queueForUnobservation(observable$1);
	return false;
}
/**
...
*/
function propagateChanged(observable$1) {
	if (observable$1.lowestObserverState_ === IDerivationState_.STALE_) return;
	observable$1.lowestObserverState_ = IDerivationState_.STALE_;
	observable$1.observers_.forEach(function(d) {
		if (d.dependenciesState_ === IDerivationState_.UP_TO_DATE_) {
			if (d.isTracing_ !== TraceMode.NONE) logTraceInfo(d, observable$1);
			d.onBecomeStale_();
		}
		d.dependenciesState_ = IDerivationState_.STALE_;
	});
}
function propagateChangeConfirmed(observable$1) {
	if (observable$1.lowestObserverState_ === IDerivationState_.STALE_) return;
	observable$1.lowestObserverState_ = IDerivationState_.STALE_;
	observable$1.observers_.forEach(function(d) {
		if (d.dependenciesState_ === IDerivationState_.POSSIBLY_STALE_) {
			d.dependenciesState_ = IDerivationState_.STALE_;
			if (d.isTracing_ !== TraceMode.NONE) logTraceInfo(d, observable$1);
		} else if (d.dependenciesState_ === IDerivationState_.UP_TO_DATE_) observable$1.lowestObserverState_ = IDerivationState_.UP_TO_DATE_;
	});
}
function propagateMaybeChanged(observable$1) {
	if (observable$1.lowestObserverState_ !== IDerivationState_.UP_TO_DATE_) return;
	observable$1.lowestObserverState_ = IDerivationState_.POSSIBLY_STALE_;
	observable$1.observers_.forEach(function(d) {
		if (d.dependenciesState_ === IDerivationState_.UP_TO_DATE_) {
...
function logTraceInfo(derivation, observable$1) {
	console.log("[mobx.trace] '" + derivation.name_ + "' is invalidated due to a change in: '" + observable$1.name_ + "'");
	if (derivation.isTracing_ === TraceMode.BREAK) {
		var lines = [];
		printDepTree(getDependencyTree(derivation), lines, 1);
		new Function("debugger;\n/*\nTracing '" + derivation.name_ + "'\n\nYou are entering this break point because derivation '" + derivation.name_ + "' is being traced and '" + observable$1.name_ + "' is now forcing it to update.\nJust follow the stacktrace you should now see in the devtools to see precisely what piece of your code is causing this update\nThe stackframe you are looking for is at least ~6-8 stack-frames up.\n\n" + (derivation instanceof ComputedValue ? derivation.derivation.toString().replace(/[*]\//g, "/") : "") + "\n\nThe dependencies for this derivation are:\n\n" + lines.join("\n") + "\n*/\n    ")();
	}
}
...
var Reaction = /* @__PURE__ */ function() {
	function Reaction$1(name_, onInvalidate_, errorHandler_, requiresObservable_) {
	...
	var _proto = Reaction$1.prototype;
	_proto.onBecomeStale_ = function onBecomeStale_() {
		this.schedule_();
	};
	...
	_proto.getDisposer_ = function getDisposer_(abortSignal) {
		var _this2 = this;
		var dispose = function dispose$1() {
			_this2.dispose();
			abortSignal == null || abortSignal.removeEventListener == null || abortSignal.removeEventListener("abort", dispose$1);
		};
	...
	return _createClass(Reaction$1, [
		{
			key: "isDisposed",
			get: function get$1() {
				return getFlag(this.flags_, Reaction$1.isDisposedMask_);
			},
			set: function set$1(newValue) {
				this.flags_ = setFlag(this.flags_, Reaction$1.isDisposedMask_, newValue);
			}
		},
		{
			key: "isScheduled",
			get: function get$1() {
				return getFlag(this.flags_, Reaction$1.isScheduledMask_);
			},
			set: function set$1(newValue) {
				this.flags_ = setFlag(this.flags_, Reaction$1.isScheduledMask_, newValue);
			}
		},
		{
			key: "isTrackPending",
			get: function get$1() {
				return getFlag(this.flags_, Reaction$1.isTrackPendingMask_);
			},
			set: function set$1(newValue) {
				this.flags_ = setFlag(this.flags_, Reaction$1.isTrackPendingMask_, newValue);
			}
		},
		{
			key: "isRunning",
			get: function get$1() {
				return getFlag(this.flags_, Reaction$1.isRunningMask_);
			},
			set: function set$1(newValue) {
				this.flags_ = setFlag(this.flags_, Reaction$1.isRunningMask_, newValue);
			}
		},
		{
			key: "diffValue",
			get: function get$1() {
				return getFlag(this.flags_, Reaction$1.diffValueMask_) ? 1 : 0;
			},
			set: function set$1(newValue) {
				this.flags_ = setFlag(this.flags_, Reaction$1.diffValueMask_, newValue === 1 ? true : false);
			}
		}
	]);
}();
...
var reactionScheduler = function reactionScheduler$1(f) {
	return f();
};
...
function setReactionScheduler(fn) {
	...
	reactionScheduler = function reactionScheduler$1(f) {
		return fn(function() {
...
function createActionFactory(autoAction$1) {
	return function action$1(arg1, arg2) {
		if (isFunction(arg1)) return createAction(arg1.name || DEFAULT_ACTION_NAME, arg1, autoAction$1);
		if (isFunction(arg2)) return createAction(arg1, arg2, autoAction$1);
		if (is20223Decorator(arg2)) return (autoAction$1 ? autoActionAnnotation : actionAnnotation).decorate_20223_(arg1, arg2);
		if (isStringish(arg2)) return storeAnnotation(arg1, arg2, autoAction$1 ? autoActionAnnotation : actionAnnotation);
		if (isStringish(arg1)) return createDecoratorAnnotation(createActionAnnotation(autoAction$1 ? AUTOACTION : ACTION, {
			name: arg1,
...
function autorun(view, opts) {
	...
	var reaction$1;
	if (runSync) reaction$1 = new Reaction(name, function() {
		this.track(reactionRunner);
	}, opts.onError, opts.requiresObservable);
	else {
		var scheduler = createSchedulerFromOptions(opts);
		var isScheduled = false;
		reaction$1 = new Reaction(name, function() {
			if (!isScheduled) {
				isScheduled = true;
				scheduler(function() {
					isScheduled = false;
	...
	if (!((_opts2 = opts) != null && (_opts2 = _opts2.signal) != null && _opts2.aborted)) reaction$1.schedule_();
	return reaction$1.getDisposer_((_opts3 = opts) == null ? void 0 : _opts3.signal);
}
var run = function run$1(f) {
	return f();
};
...
var flow = /* @__PURE__ */ Object.assign(function flow$1(arg1, arg2) {
	...
	var res = function res$1() {
		...
		var promise = new Promise(function(resolve, reject) {
			var stepId = 0;
			rejector = reject;
			function onFulfilled(res$2) {
				pendingPromise = void 0;
				var ret;
				try {
					ret = action(name + " - runid: " + runId + " - yield " + stepId++, gen.next).call(gen, res$2);
				} catch (e) {
					return reject(e);
				}
				next(ret);
...
*/
function transaction(action$1, thisArg) {
...
function whenPromise(predicate, opts) {
	...
	if (opts != null && (_opts$signal = opts.signal) != null && _opts$signal.aborted) return Object.assign(Promise.reject(/* @__PURE__ */ new Error("WHEN_ABORTED")), { cancel: function cancel$1() {
		return null;
	} });
	var cancel;
	var abort;
...
var objectProxyTraps = {
	has: function has$1(target, name) {
	...
	defineProperty: function defineProperty$1(target, name, descriptor) {
...
function makeAutoObservable(target, overrides, options) {
	...
	initObservable(function() {
		...
		if (!target[keysSymbol]) {
			...
			var keys$1 = new Set([].concat(ownKeys(target), ownKeys(proto)));
			keys$1["delete"]("constructor");
			keys$1["delete"]($mobx);
			addHiddenProp(proto, keysSymbol, keys$1);
		}
...
var ObservableArrayAdministration = /* @__PURE__ */ function() {
	function ObservableArrayAdministration$1(name, enhancer, owned_, legacyMode_) {
	...
	var _proto = ObservableArrayAdministration$1.prototype;
	_proto.dehanceValue_ = function dehanceValue_(value) {
		if (this.dehancer !== void 0) return this.dehancer(value);
		return value;
	};
	_proto.dehanceValues_ = function dehanceValues_(values$1) {
		if (this.dehancer !== void 0 && values$1.length > 0) return values$1.map(this.dehancer);
		return values$1;
	};
	_proto.intercept_ = function intercept_(handler) {
		return registerInterceptor(this, handler);
	...
	_proto.set_ = function set_(index, newValue) {
		var values$1 = this.values_;
		if (this.legacyMode_ && index > values$1.length) die(17, index, values$1.length);
		if (index < values$1.length) {
			...
			var newItems = new Array(index + 1 - values$1.length);
			for (var i = 0; i < newItems.length - 1; i++) newItems[i] = void 0;
			newItems[newItems.length - 1] = newValue;
			this.spliceWithArray_(values$1.length, 0, newItems);
		}
	};
	return ObservableArrayAdministration$1;
}();
...
var arrayExtensions = {
	...
	remove: function remove$1(value) {
		var adm = this[$mobx];
...
var ObservableMap = /* @__PURE__ */ function() {
	function ObservableMap$1(initialData, enhancer_, name_) {
		var _this = this;
	...
	var _proto = ObservableMap$1.prototype;
	...
	_proto.updateValue_ = function updateValue_(key, newValue) {
		var observable$1 = this.data_.get(key);
		newValue = observable$1.prepareNewValue_(newValue);
		if (newValue !== globalState.UNCHANGED) {
			var notifySpy = isSpyEnabled();
			var notify = hasListeners(this);
			var change = notify || notifySpy ? {
				...
				type: UPDATE,
				object: this,
				oldValue: observable$1.value_,
				name: key,
				newValue
			} : null;
			if (notifySpy) spyReportStart(change);
			observable$1.setNewValue_(newValue);
			if (notify) notifyListeners(this, change);
			if (notifySpy) spyReportEnd();
		}
	};
	_proto.addValue_ = function addValue_(key, newValue) {
		...
		transaction(function() {
			...
			var observable$1 = new ObservableValue(newValue, _this4.enhancer_, _this4.name_ + "." + stringifyKey(key), false);
			_this4.data_.set(key, observable$1);
			newValue = observable$1.value_;
	...
	_proto.values = function values$1() {
		...
		return makeIterableForMap({ next: function next() {
			var _keys$next = keys$1.next(), done = _keys$next.done, value = _keys$next.value;
			return {
				done,
				value: done ? void 0 : self$1.get(value)
			};
		} });
	};
	_proto.entries = function entries$1() {
		...
		var keys$1 = this.keys();
		return makeIterableForMap({ next: function next() {
			var _keys$next2 = keys$1.next(), done = _keys$next2.done, value = _keys$next2.value;
			return {
				done,
				value: done ? void 0 : [value, self$1.get(value)]
			};
		} });
	};
	...
	_proto.replace = function replace(values$1) {
		var _this7 = this;
		transaction(function() {
			var replacementMap = convertToMap(values$1);
	...
	*/
	...
	return _createClass(ObservableMap$1, [{
		key: "size",
		get: function get$1() {
			this.keysAtom_.reportObserved();
...
var ObservableSet = /* @__PURE__ */ function() {
	function ObservableSet$1(initialData, enhancer, name_) {
		var _this = this;
	...
	var _proto = ObservableSet$1.prototype;
	_proto.dehanceValue_ = function dehanceValue_(value) {
		if (this.dehancer !== void 0) return this.dehancer(value);
		return value;
	};
	...
	_proto.has = function has$1(value) {
		this.atom_.reportObserved();
	...
	_proto.entries = function entries$1() {
		...
		return makeIterableForSet({ next: function next() {
			var _values$next = values$1.next(), value = _values$next.value, done = _values$next.done;
			return !done ? {
				value: [value, value],
				done
			} : {
	...
	_proto.values = function values$1() {
		this.atom_.reportObserved();
		var self$1 = this;
		var values$2 = this.data_.values();
		return makeIterableForSet({ next: function next() {
			var _values$next2 = values$2.next(), value = _values$next2.value, done = _values$next2.done;
			return !done ? {
				value: self$1.dehanceValue_(value),
				done
			} : {
				value: void 0,
				done
	...
	return _createClass(ObservableSet$1, [{
		key: "size",
		get: function get$1() {
			this.atom_.reportObserved();
			return this.data_.size;
...
var ObservableObjectAdministration = /* @__PURE__ */ function() {
	function ObservableObjectAdministration$1(target_, values_, name_, defaultAnnotation_) {
	...
	var _proto = ObservableObjectAdministration$1.prototype;
	_proto.getObservablePropValue_ = function getObservablePropValue_(key) {
		return this.values_.get(key).get();
	};
	_proto.setObservablePropValue_ = function setObservablePropValue_(key, newValue) {
		var observable$1 = this.values_.get(key);
		if (observable$1 instanceof ComputedValue) {
	...
	_proto.make_ = function make_$6(key, annotation) {
	...
	_proto.extend_ = function extend_$6(key, descriptor, annotation, proxyTrap) {
		if (proxyTrap === void 0) proxyTrap = false;
		if (annotation === true) annotation = this.defaultAnnotation_;
	...
	_proto.defineObservableProperty_ = function defineObservableProperty_(key, value, enhancer, proxyTrap) {
		...
		try {
			...
			var observable$1 = new ObservableValue(value, enhancer, this.name_ + "." + key.toString(), false);
			this.values_.set(key, observable$1);
			this.notifyPropertyAddition_(key, observable$1.value_);
		} finally {
			endBatch();
		}
		return true;
	...
	_proto.delete_ = function delete_(key, proxyTrap) {
		...
		try {
			...
			var observable$1 = this.values_.get(key);
			var value = void 0;
			if (!observable$1 && (notify || notifySpy)) {
				var _getDescriptor$1;
				value = (_getDescriptor$1 = getDescriptor(this.target_, key)) == null ? void 0 : _getDescriptor$1.value;
			}
			if (proxyTrap) {
			...
			if (observable$1) {
				...
				if (observable$1 instanceof ObservableValue) value = observable$1.value_;
				propagateChanged(observable$1);
			}
...
var safariPrototypeSetterInheritanceBug = /* @__PURE__ */ function() {
	var v = false;
	var p = {};
	Object.defineProperty(p, "0", { set: function set$1() {
		v = true;
	} });
	Object.create(p)["0"] = 1;
	return v === false;
...
var StubArray = function StubArray$1() {};
...
var LegacyObservableArray = /* @__PURE__ */ function(_StubArray) {
	function LegacyObservableArray$1(initialValues, enhancer, name, owned) {
		var _this;
		if (name === void 0) name = "ObservableArray@" + getNextId();
		if (owned === void 0) owned = false;
	...
	_inheritsLoose(LegacyObservableArray$1, _StubArray);
	var _proto = LegacyObservableArray$1.prototype;
	...
	_proto[Symbol.iterator] = function() {
		...
		return makeIterable({ next: function next() {
			return nextIndex < self$1.length ? {
				value: self$1[nextIndex++],
				done: false
			} : {
				done: true,
				value: void 0
	...
	return _createClass(LegacyObservableArray$1, [{
		key: "length",
		get: function get$1() {
...
function getAtom(thing, property) {
	if (typeof thing === "object" && thing !== null) {
		...
		if (isObservableMap(thing)) {
			...
			var observable$1 = thing.data_.get(property) || thing.hasMap_.get(property);
			if (!observable$1) die(25, property, getDebugName(thing));
			return observable$1;
		}
...
function initObservable(cb) {
	...
	var allowStateChanges$1 = allowStateChangesStart(true);
	startBatch();
	try {
		return cb();
	} finally {
		endBatch();
		allowStateChangesEnd(allowStateChanges$1);
...
//#endregion
export { onBecomeUnobserved as $, getObserverTree as A, isObservable as B, flowResult as C, getDebugName as D, getAtom as E, isComputed as F, isObservableSet as G, isObservableMap as H, isComputedProp as I, makeAutoObservable as J, isObservableValue as K, isComputingDerivation as L, intercept as M, interceptReads as N, getDependencyTree as O, isAction as P, onBecomeObserved as Q, isFlow as R, flow as S, getAdministration as T, isObservableObject as U, isObservableArray as V, isObservableProp as W, observable as X, makeObservable as Y, observe as Z, computed as _, Reaction as a, runInAction as at, entries as b, action as c, toJS as ct, allowStateReadsStart as d, untracked as dt, onReactionError as et, apiDefineProperty as f, values as ft, comparer as g, autorun as h, ObservableSet as i, resetGlobalState as it, has as j, getGlobalState as k, allowStateChanges as l, trace as lt, autoAction as m, FlowCancellationError as n, reaction as nt, _endAction as o, set as ot, apiOwnKeys as p, when as pt, keys as q, ObservableMap as r, remove as rt, _startAction as s, spy as st, $mobx as t, override as tt, allowStateReadsEnd as u, transaction as ut, configure as v, get as w, extendObservable as x, createAtom as y, isFlowCancellationError as z };
//# sourceMappingURL=mobx.esm-BloO_sOI.js.map